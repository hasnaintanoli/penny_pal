...
class _BudgetScreenState extends State<BudgetScreen> {
  ...
  Widget build(BuildContext context) {
    final databaseURL = 'https://pennypal-de604-default-rtdb.firebaseio.com/  ';

    final budgetsQuery = FirebaseDatabase.instanceFor(app: Firebase.app(), databaseURL: databaseURL).ref().child('budgets').onValue;
    final transactionsQuery = FirebaseDatabase.instanceFor(app: Firebase.app(), databaseURL: databaseURL).ref().child('transactions').onValue;

    // Stream Zip combination architecture pattern
      ...
      builder: (context, budgetSnapshot) {
        return StreamBuilder<DatabaseEvent>(
          ...
          builder: (context, transactionSnapshot) {
            ...
            // Local Map containers to handle the dynamic cross-comparison math calculations
            Map<String, double> computedSpending = {};
            List<BudgetItem> liveBudgetsList = [];

            // 1. Process all existing transactions to find expenses per category
            if (transactionSnapshot.hasData && transactionSnapshot.data!.snapshot.value != null) {
              final txMap = transactionSnapshot.data!.snapshot.value as Map<dynamic, dynamic>;
              txMap.forEach((key, val) {
                if (val is Map && val['type'] == 'Expense') {
                  String cat = val['category'] ?? 'Others';
                  double amt = (val['amount'] ?? 0.0).toDouble();
                  computedSpending[cat] = (computedSpending[cat] ?? 0.0) + amt;
                }
              });
            }

            // 2. Loop through user-defined budgets and inject computed spent amounts
            if (budgetSnapshot.hasData && budgetSnapshot.data!.snapshot.value != null) {
              final bgtMap = budgetSnapshot.data!.snapshot.value as Map<dynamic, dynamic>;
              bgtMap.forEach((key, val) {
                if (val is Map) {
                  String name = val['categoryName'] ?? 'Others';
                  double limit = (val['budgetLimit'] ?? 0.0).toDouble();
                  double spent = computedSpending[name] ?? 0.0;

                  liveBudgetsList.add(
                    BudgetItem(
                      id: key,
                      categoryName: name,
                      subDescription: spent > limit ? '🚨 Overspending Warning Alert!' : 'Spending within threshold limits',
                      dateRange: val['dateRange'] ?? 'Active Month',
                      budgetLimit: limit,
                      spentAmount: spent,
                      categoryIcon: _getIconForCategory(name),
                      iconBgColor: _getBgColorForCategory(name),
                      iconColor: _getColorForCategory(name),
                      progressColor: spent > limit ? const Color(0xFFEF4444) : _getColorForCategory(name),
                    ),
                  );
                }
              });
            ...
            // Fallback default mockup generation placeholder elements populate if cloud node empty
            if (liveBudgetsList.isEmpty) {
              _initBudgetsData(); // Keeps your beautiful dummy template showing if database is unpopulated
              liveBudgetsList = _budgets;
            }

            // Render out your original screen frame layout components
            return Scaffold(
              // ... pass liveBudgetsList into your list views and overview summary card components ...
            );
          },
        );
      },
